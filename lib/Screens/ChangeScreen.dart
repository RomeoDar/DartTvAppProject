...
class ChangeScreen extends StatefulWidget {
  ...
  final String tvName;
  final String imgUrl;
  ChangeScreen(this.docID,this.tvName,this.imgUrl);
...
class _ChangeScreenState extends State<ChangeScreen> {
  ...
  Future setUserData(Map<String, dynamic> userData,BuildContext context) async {
    if(check==true || nameCtlr.text.length>0)
      {
        if(nameCtlr.text.isEmpty)
          {
            userData.addAll({'UserName': widget.tvName});
    ...
    child: SingleChildScrollView(
        child: Column(
          ...
          children: <Widget>[
            ...
            ListTile(
              title: Text(
                "Change Tv Screen Name",
                style: TextStyle(fontSize: 15,fontWeight: FontWeight.bold),
              ),
              subtitle: CupertinoTextField(
                controller: nameCtlr,
                cursorColor: primaryColor,
                placeholder: widget.tvName,
                padding: EdgeInsets.all(10),
                onChanged: (text) {
  ...
  Future getImage(
    ...
    //{
    //await uploadFile(
    //  await compressimage(image),index, isProfilePicture);
    //}
    if (image != null) {
